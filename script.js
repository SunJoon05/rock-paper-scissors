...
function updatePlayerInformation(current, value) {
    ...
    current.screen.appendChild(showIcon(value).icon.cloneNode(true));
}
...
function playGame() {
    ...
    const first_player = { screen: document.querySelector(".first-player"), choice: null, score: 0 };
    const second_player = { screen: document.querySelector(".second-player"), choice: null, score: 0 };
    let round = 0;
    ...
    function getSelectedOptions(event) {
        ...
        checkWinnerRound(first_player, second_player);
        console.log(round);

        if (round === 5) {
            winnerGame(first_player.score, second_player.score);
            round = 0;
...
function winnerGame(first_score, second_score) {
    ...
    if (first_score > second_score) {
        console.log('Humano Ganó');
    } else {
        console.log('Computadora Ganó');
    }
}
